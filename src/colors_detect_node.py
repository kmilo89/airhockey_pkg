...
def draw(mask, color, image):
    ...
    for c in contours:
        ...
        if area > 3000:
            ...
            cv2.putText(image, 'x:{}, y:{}'.format(x, y), (x+10, y), font,  0.75, (0, 0, 0), 1, cv2.LINE_AA)
            area_object = area
...
def colors_detect(img):
    ...
    img, data.pos_puck.x, data.pos_puck.y, data.found_puck, data.area_puck = draw(maskBlue, blue, img)
    img, data.pos_disk.x, data.pos_disk.y, data.found_disk, data.area_disk = draw(maskRed, red, img)
    #new_img.data = cv2.imencode('.jpg', img)[1].tostring()
    #bridge = CvBridge()
    #new_img.data = bridge.imgmsg_to_cv2(img, cv2.CV_8UC1)[:, :, ::-1]
    #new_img.data = img
